...
namespace ShoppingV2.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly ShoppingV2DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = ShoppingV2Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(ShoppingV2DbContext context)
        {
            _context = context;
        }
